...
import { validateRequest, blogBulkActionSchema } from '@/lib/validation';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user.permissions?.includes(requiredPerm)) {
      return forbiddenResponse(res, `Insufficient permissions to ${req.body.action} blogs`);
    }

    const validation = validateRequest(blogBulkActionSchema, req.body);
    if (!validation.success) {
      return errorResponse(res, 'Validation failed', 400, validation.errors);
    }

    const { ids, action } = validation.data;
    const result = await BlogService.bulkAction({ ids, action }, extractRequestMetadata(req, tokenPayload.userId));
