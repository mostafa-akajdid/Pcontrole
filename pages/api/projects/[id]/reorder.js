...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user.permissions?.includes('projects.update')) {
      return forbiddenResponse(res, 'Insufficient permissions to reorder project images');
    }
    ...
    const result = await ProjectService.reorderImages(id, validation.data.imageIds);
