...
export class DashboardService {
  ...
  static async getOverview(userId) {
    ...
    const [stats, recentProjects, recentBlogs, recentActivity, recentUsers, recentMedia, contentSummary, activityTrend, categoryBreakdown, storageBreakdown, systemHealth, userGrowth, recentNotifications, recentAuditLogs] = await Promise.all([
      ...
      safeQuery(this.getRecentBlogs(5)),
      safeQuery(this.getRecentActivity(15)),
      safeQuery(this.getRecentUsers(5)),
      safeQuery(this.getRecentMedia(5)),
      safeQuery(this.getContentSummary()),
      safeQuery(this.getActivityTrend(30)),
      safeQuery(this.getCategoryBreakdown()),
      safeQuery(this.getStorageBreakdown()),
      safeQuery(this.getSystemHealth()),
      safeQuery(this.getUserGrowth(30)),
      safeQuery(prisma.notification.findMany({ where: { userId, deletedAt: null }, take: 5, orderBy: { createdAt: 'desc' } })),
      safeQuery(prisma.auditLog.findMany({ take: 5, orderBy: { createdAt: 'desc' }, include: { user: { select: { id: true, name: true, email: true, avatar: true } } } })),
    ]);
