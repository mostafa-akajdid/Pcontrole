...
export class ProjectCategoryService {
  ...
  static async generateUniqueSlug(name, excludeId = null) {
    ...
    const existing = await prisma.projectCategory.findFirst({ where });
  ...
  static async delete(id) {
    ...
    if (projectCount > 0) {
      throw new Error(
        `Category '${category.name}' is assigned to ${projectCount} project${projectCount !== 1 ? 's' : ''} and cannot be deleted.`
      );
    }
